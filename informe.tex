...
\hypersetup{
    ...
    filecolor=magenta,      
    urlcolor=cyan,
    pdftitle={Proyecto Sistemas Operativos y Distribuidos - Informe},
...
\pagestyle{fancy}
\fancyhf{}
\rhead{Proyecto Sistemas Operativos y Distribuidos}
\lfoot{Integrantes: Hernandez, Muñoz, Martines, Paredes}
\rfoot{Página \thepage}

\title{Infraestructura y simulación de mensajería en tiempo real para entornos de salud digital\\
\large Proyecto Sistemas Operativos y Distribuidos}
\author{Krisstal Hernandez \and Mariano Muñoz \and Felipe Martines \and Maximiliano Paredes}
...
\begin{itemize}
    ...
    \item Implementar un mecanismo de replicación lógica bidireccional asíncrona sobre bases de datos relacionales PostgreSQL 16 con filtros condicionales para evitar colisiones y propagación circular.
    \item Desarrollar e integrar microservicios basados en Node.js que utilicen la tecnología Socket.io v4.7.5 para la transmisión de eventos clínicos bidireccionales en tiempo real.
    \item Configurar seguridad perimetral mediante un proxy inverso Nginx que gestione de manera segura la negociación de protocolos (WebSockets Upgrade), aplique políticas de limitación de tasa (Rate Limiting) y oculte la firma de los componentes internos.
\end{itemize}
...
En lugar de utilizar replicación física (que bloquea la base de datos de réplica en modo de solo lectura y requiere duplicar toda la información de disco del clúster), se implementa **Replicación Lógica**. Esta estrategia distribuye las transacciones mediante un mecanismo publicador/suscriptor. 
...
\textbf{Descripción del flujo:} Al insertarse el registro en \texttt{db-nube} con el valor de metadato \texttt{origen\_registro = 'nube'}, el motor de replicación procesa el WAL y envía la fila a la base de datos local. Al ingresar como Médico en una Estación Clínica de la red local y consultar el RUT ingresado, el sistema local encuentra el registro directamente en \texttt{db-local} y carga la ficha en pantalla. Seguidamente, al registrar un diagnóstico médico, la Estación realiza un UPDATE local en la tabla y esta actualización de diagnóstico clínico se replica en reversa de forma transparente hacia la base de datos core en la nube.

\subsection{Escenario 3: Resiliencia ante caídas de conectividad}
\textbf{Descripción del flujo:} Con el sistema en operación, se simula la caída de conectividad WAN deteniendo el contenedor de la nube (\texttt{db-nube}). A pesar de no haber conexión con la nube, el hospital mantiene su operación normal. El médico puede seguir consultando fichas y registrando diagnósticos locales en \texttt{db-local}. El motor local registra estos cambios en el log del WAL lógico y mantiene los datos retenidos en su slot de replicación. Una vez restablecida la conexión WAN, la suscripción local se reconecta automáticamente, consume el backlog del slot y sincroniza todos los cambios pendientes sin pérdida de consistencia.
